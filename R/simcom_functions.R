...
plot_polygons <- function(data,
  ...
  is.num     <-  apply(data, 2, is.numeric)
...
sim_data <- function(n_pop = 1000,
  ...
  rawvars <- mvrnorm(n=n_pop, mu=mu, Sigma=Sigma)
  ...
  simulate_l1_bs <- function(mu_S_person, mu_B_person, cor_bs_l1 = .2,
    ...
    l1_bs_raw <- mvrnorm(n = l1_bs_population,
  ...
  level_1_id_states <- map2(.x = level1_states,
                            .y = id,
                            .f = function(x, y) {
                              cidd(x, y)
                              }
...
cons_comp_analysis <- function(data,
  ...
  sample_all_o <- function(i){ # rsquare
    ...
    # aggregate states
    aggregate_states <- map(odf, aggregate_within)
    ...
    apsOut = aps(level2_data, outcome_state, list(pred_trait[1], pred_state,
                                                  pred_trait[2]))
